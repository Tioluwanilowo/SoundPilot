...
export const vocalSuggestionRules: SuggestionRule[] = [
  ...
  // ── Low-mid mud cut for vocals ──────────────────────────────────────────────
  {
    ...
    recommend: (s) => {
      ...
      const amount = Math.max(-8, -(excess * 0.4)) // Scale cut to excess, max 8 dB
      return {
        id: 'vocal-lowmid-mud',
        action: 'cut',
        frequency: 280,
        amount: Math.round(amount * 2) / 2, // Round to nearest 0.5 dB
        bandwidth: 'medium',
  ...
  // ── Presence boost for clarity ──────────────────────────────────────────────
  {
    ...
    condition: (s) =>
      ...
      !s.issues.some(i => i.type === 'harshness'), // Don't boost if already harsh
    recommend: (_s) => ({
      id: 'vocal-presence-boost',
      action: 'boost',
      frequency: 5000,
